...
from clindaws.execution.decompression import (
    split_output_ports_kcluster,
    split_output_ports_one_to_n,
)
...
DecompressionMode = str | None


@dataclass(frozen=True)
class OptimizedCandidateOptimizationResult:
...
def optimize_candidate_surface(
    config: SnakeConfig,
    ontology: Ontology,
    tools: tuple[ToolMode, ...],
    *,
    max_workers: int = 1,
    decompression_mode: DecompressionMode = None,
) -> OptimizedCandidateOptimizationResult:
    ...
    if decompression_mode not in {None, "kcluster", "one-to-n"}:
        raise ValueError(f"Unsupported decompression mode: {decompression_mode}")
    ...
    decompression_stats: dict[str, int] = {}
    if decompression_mode == "kcluster":
        decompression_stats.update(
            split_output_ports_kcluster(
                relevant_records=relevant_records,
                bindable_pairs=bindable_pairs,
                relevant_candidates=frozenset(backward_relevant_candidates | direct_goal_candidates),
            )
        )
    elif decompression_mode == "one-to-n":
    ...
    return OptimizedCandidateOptimizationResult(
