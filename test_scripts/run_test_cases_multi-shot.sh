...
for config in "${REGULAR_CASES[@]}"; do
  echo "==> single-shot: $config"
  python -m clindaws "$config" --mode single-shot
  echo "==> multi-shot: $config"
  python -m clindaws "$config" --mode multi-shot
done
