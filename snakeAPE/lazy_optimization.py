"""Lazy-mode precomputation and optimization pipeline."""
...
from .models import SnakeConfig, ToolExpansionStat, ToolMode
...
class LazyOptimizationResult:
    """Optimized lazy candidate surface ready for fact emission."""
...
def _factor_signature_support_classes(
    ...
    return (
        ...
        {
            "lazy_signature_supports_raw": raw_signature_supports,
            "lazy_signature_support_classes": factored_supports,
            "lazy_signature_supports_collapsed": raw_signature_supports - factored_supports,
            "lazy_support_class_bindable_ports_emitted": sum(
                len(producer_ports)
...
def optimize_lazy_candidates(
    config: SnakeConfig,
    ontology: Ontology,
    tools: tuple[ToolMode, ...],
) -> LazyOptimizationResult:
    """Precompute the lazy candidate surface before fact emission."""

    roots = core._build_roots(config, ontology)
    resolver = core._ExpansionResolver(ontology, roots, "python")
    tool_stats: list[ToolExpansionStat] = []
    candidate_records: list[dict[str, object]] = []

    lazy_offset: dict[str, int] = defaultdict(int)

    forbidden_tool_ids = core._collect_lazy_forbidden_tool_ids(config, ontology, tools)
    ...
    for tool in candidate_source_tools:
        ...
        for port_idx, port in enumerate(tool.inputs):
            port_values, port_variant_count = core._lazy_port_expansion(
                resolver,
                core._normalize_dim_map(port.dimensions),
                expand_outputs=False,
            )
            port_values_by_dimension = core._group_port_values_by_dimension(port_values)
            workflow_input_matches = [
                wf_index
                for wf_index, workflow_input in enumerate(config.inputs)
                if core._workflow_input_matches_lazy_port(ontology, workflow_input, port_values_by_dimension)
            ]
            input_ports.append(
                {
                    "port_idx": port_idx,
        ...
        for port_idx, port in enumerate(tool.outputs):
            declared_dims = core._normalize_dim_map(port.dimensions)
            port_values, port_variant_count = core._lazy_port_expansion(
                resolver,
                declared_dims,
                expand_outputs=True,
        ...
        candidate_records.append(
            {
                ...
                "output_ports": core._compress_duplicate_output_ports(tuple(output_ports)),
            }
        )
    ...
    for goal_item in config.outputs:
        ...
        for dim, values in sorted(goal_item.items()):
            goal_dims[dim] = core._dedupe_stable(
                expanded_value
                for value in values
    ...
    for record in candidate_records:
        ...
        for output_port in tuple(record["output_ports"]):
            ...
            if any(core._lazy_output_matches_lazy_input_fset(output_fset, gf) for gf in goal_fsets):
        ...
        for output_port in tuple(producer_record["output_ports"]):
            ...
            for consumer_record in candidate_records:
                ...
                for input_port in tuple(consumer_record["input_ports"]):
                    ...
                    if core._lazy_output_matches_lazy_input_fset(output_fset, input_port["port_values_fset"]):
                        bindable_pairs.add(
    ...
    for record in candidate_records:
        ...
        for input_port in tuple(record["input_ports"]):
            ...
            if any(
                core._workflow_input_matches_lazy_port(
                    ontology,
                    workflow_input,
    ...
    if config.use_all_generated_data == "ALL":
        backward_relevant_candidates, min_anchor_distance_by_candidate = core._collect_lazy_backward_relevant_candidates(
            config,
            ontology,
            tools,
            candidate_records=(
    ...
    signature_profiles_by_id, _profile_values_by_id, profile_accepts_by_id, lazy_schema_stats = core._assign_lazy_signature_profiles(
        ontology,
        roots,
    ...
    for record in relevant_records:
        ...
        for output_port in tuple(record["output_ports"]):
            ...
            compression_cache_key = (
                core._lazy_dim_values_cache_key(output_port["port_values_by_dimension"]),
                preserve_goal_profiles,
            )
            compressed_vals = output_compression_cache.get(compression_cache_key)
            if compressed_vals is None:
                compressed_vals = core._compress_lazy_output_choice_values(
                    ontology,
        ...
        record["output_ports"] = core._compress_duplicate_output_ports(tuple(compressed_output_ports))

    compressed_reverse_edges, compressed_produced_bindable_ports, signature_bindable_ports, bindability_stats = (
        core._collect_compressed_lazy_bindability_surface(
            bindable_pairs,
    ...
    for record in relevant_records:
        ...
        goals_satisfied = all(
            any(
                core._lazy_output_matches_lazy_input_fset(
                    output_port["port_values_fset"],
                    gf,
                )
    ...
    min_step_by_candidate = core._compute_lazy_candidate_min_steps(
        relevant_records,
    ...
    must_use_min_steps, at_step_lower_bounds = core._collect_lazy_selector_lower_bounds(
        config,
        ontology,
        tools,
        tool_min_steps=tool_min_steps,
    )
    exact_prefix_lower_bound = core._collect_lazy_exact_prefix_lower_bound(
        config,
        ontology,
        tools,
